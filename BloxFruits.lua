...
local function saveKey(k)
    ...
    pcall(function() writefile(SAVE_FILENAME, tostring(k)) end)
end
local function loadSavedKey()
    if not canWriteFiles() then return nil end
    if not isfile(SAVE_FILENAME) then return nil end
    local ok, data = pcall(readfile, SAVE_FILENAME)
    if ok then return data end
    return nil
end

-- small helper to trim whitespace from strings
...
btn.MouseButton1Click:Connect(function()
	local key = box.Text
	-- trim whitespace from typed key so accidental spaces don't break validation
	key = trim(key)
	if key == CORRECT_KEY then
        status.TextColor3 = Color3.fromRGB(100,255,120)
        status.Text = "✔️ Key đúng, đang load..."
        TweenService:Create(frame, TweenInfo.new(0.6), {Position = UDim2.new(0.5, -210, -0.5, 0)}):Play()
        task.wait(0.6)
		-- save the (trimmed) key so next time user won't need to re-enter
		pcall(function() saveKey(key) end)
        runCode()
    else
        status.TextColor3 = Color3.fromRGB(255,120,120)
        status.Text = "❌ Sai key, vui lòng thử lại."
        TweenService:Create(frame, TweenInfo.new(0.1), {Position = UDim2.new(0.5, -200, 0.5, -120)}):Play()
        task.wait(0.05)
        TweenService:Create(frame, TweenInfo.new(0.1), {Position = UDim2.new(0.5, -210, 0.5, -120)}):Play()
    end
...
-- If a key was stored previously and it matches, auto-run the hub
do
    local saved = nil
    pcall(function() saved = loadSavedKey() end)
	if saved then
		-- trim whitespace from saved key
		saved = trim(saved)
		-- prefill the input box so user can see the stored key (optional)
		pcall(function() box.Text = saved end)
	end
    if saved and saved == CORRECT_KEY then
        -- show positive status briefly then run
        status.TextColor3 = Color3.fromRGB(100,255,120)
        status.Text = "✔️ Key đã lưu, đang load..."
        TweenService:Create(frame, TweenInfo.new(0.6), {Position = UDim2.new(0.5, -210, -0.5, 0)}):Play()
        task.wait(0.6)
        runCode()
    end
...
-- Forget key button (so user can clear saved key without touching executor files)
local forgetBtn = Instance.new("TextButton", frame)
forgetBtn.Size = UDim2.new(0,100,0,28)
forgetBtn.Position = UDim2.new(0.5, 70, 0.75, 0)
forgetBtn.BackgroundColor3 = Color3.fromRGB(200,80,80)
forgetBtn.TextColor3 = Color3.fromRGB(255,255,255)
forgetBtn.Font = Enum.Font.Gotham
forgetBtn.TextSize = 14
forgetBtn.Text = "Quên key"
forgetBtn.AutoButtonColor = false
Instance.new("UICorner", forgetBtn).CornerRadius = UDim.new(0,8)
forgetBtn.MouseButton1Click:Connect(function()
    forgetKey()
    status.TextColor3 = Color3.fromRGB(255,200,100)
    status.Text = "🔒 Key đã bị xoá"
end)
